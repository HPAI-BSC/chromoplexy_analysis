'''
Compute the number of cliques of size 3 and 4 for the data of the different cancer types.
Then aggregate that information for all cancer types. Cliques are searched in a graph generated with variable
window size, from 100 to 1M.
'''
...
for w in [100, 1000, 10000, 100000, 1000000]:
    ...
    for t in cancer_types.keys():
        ...
        ax[plot_position_x, plot_position_y].set_xticklabels((list(map(str, np.arange(30))) + ['>=30'])[::2],
                                                             fontsize=18)
    ...
    for t in cancer_types.keys():
        ...
        ax[plot_position_x, plot_position_y].set_xticks(x_ticks[1::2], minor=True)
        # For the major, add the label
        ax[plot_position_x, plot_position_y].set_xticklabels((list(map(str, np.arange(10))) + ['>=30'])[::2],
                                                             fontsize=18)
        # Add title to subplot of multiplot-plot
        ax[plot_position_x, plot_position_y].set_title(t, fontsize=30)
        # Print the ticks, both major and minor, of different size
